...
| KubeServer | `yunionio/kubecomps` tag `v4.0.3`, commit `eb381ed38b587e2cabd081c69a0e6b0aa04a5360`, built inside the pinned RISC-V container under QEMU user-mode on x86_64 with the three kubespray archives recorded in `versions.env`; its CGO/Ceph link uses Alpine `lld20` `20.1.8-r0` from the measured Aliyun Alpine mirror because GNU ld 2.44 crashes on the generated RISC-V relocation stream |
...
the required execution mode and final `riscv64` architecture, and publish RPM
and image digest lists as build artifacts.
